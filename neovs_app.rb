...
$neo = Neography::Rest.new("http://AlumnosDB:sYvd8vGnswmF4bpjIVZZ@alumnosdb.sb02.stations.graphenedb.com:24789")
...
class App < Sinatra::Base
  ...
  post '/graph' do
    ...
    cypher = prepare_cypher(match,where)
    cypher << " RETURN EXTRACT(n in nodes(p) | [ID(n), COALESCE(n.nombre, n.title, ID(n)), LABELS(n)]), 
  ...
  get '/related/:id' do
    ...
    results = connections.collect{|n| {"source" => n[0], "source_data" => {:name => n[2]["data"]["nombre"] || n[2]["data"]["title"] || n[2]["data"].first, 
                                                                 :description => n[2],
                                                                 :type => n[1].first },
                             "target" => n[3], "target_data" => {:name => n[5]["data"]["nombre"] || n[5]["data"]["title"] || n[5]["data"].first,
                                                                 :description => n[5],
                                                                 :type => n[4].first}} }
     results.to_json                                               
  end  
